...
try:
    ...
    # Truncar la tabla fact_poblacion antes de insertar nuevos datos
    with engine.connect() as connection:
        connection.execute(text("TRUNCATE TABLE verin_dw.fact_poblacion"))
        connection.commit()

    # Insertar datos en la tabla fact_poblacion
    df_pivot.to_sql('fact_poblacion', engine, schema='verin_dw', if_exists='append', index=False)    

    print("ETL de fact_poblacion completado con éxito.")
except Exception as e:
    print(f"Error en el ETL de fact_poblacion: {e}")
    exit(1)
...
try:
    ...
    # Truncar la tabla fact_nacimientos antes de insertar nuevos datos
    with engine.connect() as connection:
        connection.execute(text("TRUNCATE TABLE verin_dw.fact_nacimientos"))
        connection.commit()

    # Insertar/actualizar en la tabla fact_nacimientos
    df_pivot.to_sql('fact_nacimientos', engine, schema='verin_dw', if_exists='append', index=False)

    print("ETL de fact_nacimientos completado con éxito.")
except Exception as e:
    print(f"Error en el ETL de fact_nacimientos: {e}")
    exit(1)
...
try:
    ...
    # Truncar la tabla fact_defunciones antes de insertar nuevos datos
    with engine.connect() as connection:
        connection.execute(text("TRUNCATE TABLE verin_dw.fact_defunciones"))
        connection.commit()

    # Insertar/actualizar en la tabla fact_defunciones
    df_pivot.to_sql('fact_defunciones', engine, schema='verin_dw', if_exists='append', index=False)

    print("ETL de fact_defunciones completado con éxito.")
except Exception as e:
    print(f"Error en el ETL de fact_defunciones: {e}")
    exit(1)
"""
...
try:
    ...
    # Truncar la tabla fact_poblacion_edad antes de insertar nuevos datos
    with engine.connect() as connection:
        connection.execute(text("TRUNCATE TABLE verin_dw.fact_poblacion_edad"))
        connection.commit()

    # Insertar datos en la tabla fact_poblacion_edad
    df_pivot.to_sql('fact_poblacion_edad', engine, schema='verin_dw', if_exists='append', index=False)

    print("ETL de fact_poblacion_edad completado con éxito.")
except Exception as e:
    print(f"Error en el ETL de fact_poblacion_edad: {e}")
    exit(1)
...
try:
    ...
    # Truncar la tabla fact_poblacion_lugar antes de insertar nuevos datos
    with engine.connect() as connection:
        connection.execute(text("TRUNCATE TABLE verin_dw.fact_poblacion_lugar"))
        connection.commit()

    # Insertar datos en la tabla fact_poblacion_lugar
    df_pivot.to_sql('fact_poblacion_lugar', engine, schema='verin_dw', if_exists='append', index=False)

    print("ETL de fact_poblacion_lugar completado con éxito.")
except Exception as e:
    print(f"Error en el ETL de fact_poblacion_lugar: {e}")
    exit(1)